...
func solve() {
	...
	rbt.Put(-1000, -1000)
	rbt.Put(MaxInt, MaxInt)
	for i := 0; i < n; i++ {
		A := nextInt()
		m[A]++
		a[i] = A
		floor, _ := rbt.Floor(A)
		fl, fr := floor.Key.(int), floor.Value.(int)
		if fl <= A && A <= fr {
			continue
		}
		ceil, _ := rbt.Ceiling(A)
		cl, cr := ceil.Key.(int), ceil.Value.(int)
		if fr+1 == A && cl-1 == A {
			rbt.Remove(cl)
			rbt.Put(fl, cr)
		} else if fr+1 == A {
	...
	for q := 0; q < Q; q++ {
		...
		if a[i] == x {
			...
			if !ok {
				...
				out.Println(value.(int) + 1)
			}
			continue
		}
		if m[a[i]] == 1 {
			floor, _ := rbt.Floor(a[i])
			l := floor.Key.(int)
			r := floor.Value.(int)
			if a[i] != l && a[i] != r {
				rbt.Put(l, a[i]-1)
