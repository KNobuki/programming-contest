...
func solve() {
	...
	for ; Q > 0; Q-- {
		...
		case 1:
			x, v := nextInt2()
			x--
			seg.Update(x, x+1, M{v})
		case 2:
			l, r := nextInt2()
			l--
			out.Println(seg.Query(l, r).val)
		case 3:
			x, v := nextInt2()
			ans := seg.FindLeftest(x, n, v)
			if ans == -1 {
				out.Println(n + 1)
			} else {
				out.Println(ans + 1)
...
func (sg *SegTreeLazy) FindLeftest(l, r, v int) int {
	return sg.findLeftestSub(0, 0, sg.Size, l, r, v)
}

func (sg *SegTreeLazy) findLeftestSub(k, sl, sr, l, r, v int) int {
	sg.Eval(k)
	if sr <= l || r <= sl || sg.Dat[k].val < v {
		return -1
	}
	if sr-sl == 1 {
		return sl
	...
	sm := (sl + sr) / 2
	left := sg.findLeftestSub(2*k+1, sl, sm, l, r, v)
	if left != -1 {
		return left
	}
	return sg.findLeftestSub(2*k+2, sm, sr, l, r, v)
}
