...
func solve() {
	...
	var dfs func(now int, ans string, remain int)
	dfs = func(now int, ans string, remain int) {
		if now == n {
			if !set[ans] && len(ans) >= 3 {
				out.Println(ans)
				out.writer.Flush()
				os.Exit(0)
			}
		...
		if now != 0 && ans[len(ans)-1] != '_' {
			dfs(now, ans+"_", remain)
		} else {
			dfs(now+1, ans+s[a[now]], remain)
			if now != 0 && remain != 0 {
				dfs(now, ans+"_", remain-1)
			}
		}
	}
	remain := 16
