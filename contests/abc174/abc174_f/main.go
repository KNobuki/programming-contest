...
func solve() {
	...
	sort.Slice(sortedQ, func(i, j int) bool {
		return sortedQ[i].r < sortedQ[j].r
	})
	r := 0
	ans := make(map[lr]int, Q)
	p := make([]int, n+1)
	for i := 0; i < n+1; i++ {
		p[i] = -1
	}
	for _, v := range sortedQ {
		for r < n && r <= v.r {
			if p[c[r]] != -1 {
				seg.Update(p[c[r]], p[c[r]]+1, M{0})
			}
			seg.Update(r, r+1, M{1})
			p[c[r]] = r
			r++
		}
		ans[lr{v.l, v.r}] = seg.Query(v.l, v.r+1).val
	}
	for _, v := range q {
		out.Println(ans[lr{v.l, v.r}])
	}
}
