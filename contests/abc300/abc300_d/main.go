...
func solve() {
	n := nextInt()
	era := eratosthenes(1e6)
	primes := make([]int, 0, 1e6)
	for i := 1; i <= 1000000/4+40000; i++ {
		if era[i] {
	...
	for i := 0; i < len(primes); i++ {
		for j := i + 1; j < len(primes); j++ {
			if primes[j] <= n && primes[j]*primes[j] <= n && primes[i]*primes[i]*primes[j] <= n && primes[i]*primes[j]*primes[j] <= n {
				for k := i + 1; k < j; k++ {
					if primes[k]*primes[j] <= n && primes[i]*primes[k]*primes[j] <= n && primes[i]*primes[i]*primes[k] <= n && primes[k]*primes[j]*primes[j] <= n && primes[i]*primes[i]*primes[k]*primes[j] <= n && primes[i]*primes[k]*primes[j]*primes[j] <= n && primes[i]*primes[i]*primes[k]*primes[j]*primes[j] <= n {
						ans++
					} else {
						break
					}
...
func eratosthenesWithMinFactor(n int) []int {
	prime := make([]bool, n+1)
