...
func solve() {
	...
	// 1 takahashi 0 unselected -1 aoki
	color := make([][]int, 3)
	for i := 0; i < 3; i++ {
		color[i] = make([]int, 3)
	}
	// 1 takahashi 0 undeterminated -1 aoki
	checkWin := func() int {
		for i := 0; i < 3; i++ {
			tCol, aCol, tRow, aRow := 0, 0, 0, 0
			for j := 0; j < 3; j++ {
				if color[i][j] == 1 {
					tCol++
				}
	...
	var dfs func(takahashi bool) bool
	dfs = func(takahashi bool) bool {
		w := checkWin()
		if w == 1 {
			return true
		}
