...
func solve() {
	...
	if k%2 == 0 {
		...
		for i := 1; i < k; i++ {
			if i%2 == 1 {
				even = append(even, a[i]-a[i-1])
			} else {
				odd = append(odd, a[i]-a[i-1])
			}
		}
		evenCum := []int{0, even[0]}
		oddCum := []int{0, odd[0]}
		for i := 1; i < len(even); i++ {
			evenCum = append(evenCum, evenCum[i]+even[i])
		}
		for i := 1; i < len(odd); i++ {
			oddCum = append(oddCum, oddCum[i]+odd[i])
		}
		ans := min(oddCum[len(oddCum)-1], evenCum[len(evenCum)-1])
		for i := 2; i < k; i += 2 {
			ans = min(ans, evenCum[i/2]+(oddCum[len(oddCum)-1]-oddCum[i/2]))
		}
		out.Println(ans)
	}
}
