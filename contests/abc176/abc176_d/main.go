...
func solve() {
	...
	que := list.New()
	que.PushBack(edge{cy, cx})
	dist[cy][cx] = 0
	d := []int{1, 0, -1, 0}
	for que.Len() > 0 {
		now := que.Front().Value.(edge)
		que.Remove(que.Front())
		for i := 0; i < 4; i++ {
			ny, nx := now.y+d[i], now.x+d[(i+1)%4]
			if ny < 0 || ny >= h || nx < 0 || nx >= w || s[ny][nx] == '#' || dist[ny][nx] <= dist[now.y][now.x] {
				continue
			}
			dist[ny][nx] = dist[now.y][now.x]
			que.PushFront(edge{ny, nx})
		}
		for y := -2; y <= 2; y++ {
			for x := -2; x <= 2; x++ {
				ny, nx := now.y+y, now.x+x
				if ny < 0 || ny >= h || nx < 0 || nx >= w || s[ny][nx] == '#' || dist[ny][nx] <= dist[now.y][now.x]+1 {
					continue
				}
				dist[ny][nx] = dist[now.y][now.x] + 1
				que.PushBack(edge{ny, nx})
			}
		}
	}
	if dist[dy][dx] == 1e9 {
