...
func solve() {
	...
	for h.Len() > 0 {
		...
		for _, v := range g[now.To] {
			...
			heap.Push(h, Edge{To: v, Num: a[v], Cnt: tmp})
		}
	}
	out.Println(cnt[n-1])
}
...
func dijkstraWithPath(N int, start int, graph [][]Edge) ([]int, []int) {
	...
	for h.Len() > 0 {
		...
		for _, p := range graph[position] {
			...
			to, weight := p.To, dist[position]+p.Num
			// 現在のノードから行った場合に、他のノードから行った場合より距離が短い場合
			if dist[to] > weight {
				// 最短距離リスト更新
				dist[to] = weight
				// 確定候補キューに格納
				heap.Push(h, Edge{Num: dist[to], To: to})
				// 経路リスト更新
				from[to] = position
			}
		}
