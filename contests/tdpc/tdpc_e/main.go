...
func solve() {
	...
	for i := 0; i < len(n); i++ {
		for j := 0; j < d; j++ {
			for k := 0; k < 10; k++ {
				dp[i+1][1][(j+k)%d] = madd(dp[i+1][1][(j+k)%d], dp[i][1][j])
				if k < int(n[i]-'0') {
					dp[i+1][1][(j+k)%d] = madd(dp[i+1][1][(j+k)%d], dp[i][0][j])
				}
				if k == int(n[i]-'0') {
					dp[i+1][0][(j+k)%d] = madd(dp[i+1][0][(j+k)%d], dp[i][0][j])
				}
			}
		}
	}
	out.Println(dp[len(n)][0][0] + dp[len(n)][1][0] - 1)
}
