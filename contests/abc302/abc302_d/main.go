...
func solve() {
	...
	sort.Ints(b)
	ai, bi := 0, 0
	for ai, bi = 0, 0; ai < n; ai++ {
		for bi < m && b[bi]-a[ai] <= d {
			bi++
		}
		if bi > 0 && abs(a[ai]-b[bi-1]) <= d {
			ans = max(ans, a[ai]+b[bi-1])
		}
	}
	for ai, bi = 0, 0; bi < m; bi++ {
		for ai < n && a[ai]-b[bi] <= d {
			ai++
		}
		if ai > 0 && abs(a[ai-1]-b[bi]) <= d {
			ans = max(ans, a[ai-1]+b[bi])
		}
	}
	out.Println(ans)
}
