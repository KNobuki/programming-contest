...
func solve() {
	...
	for i := 0; i < m; i++ {
		...
		e[i] = Edge{From: u - 1, To: v - 1, Weight: w}
	}
	ans := k
	var dfs func(i int, edges []int)
	dfs = func(i int, edges []int) {
		if len(edges) == n-1 {
			dsu := NewDsu(n)
			sum := 0
			for _, v := range edges {
				if dsu.Same(e[v].From, e[v].To) {
					return
				}
				dsu.Merge(e[v].From, e[v].To)
				sum += e[v].Weight
			}
			ans = min(ans, sum%k)
			return
		...
		if m-i < n-1-len(edges) {
			return
		}
		dfs(i+1, append(edges, i))
		dfs(i+1, edges)
	}
	dfs(0, []int{})
	out.Println(ans)
	...
	for h.Len() > 0 {
		...
		for _, p := range graph[position] {
			...
			if dist[to] > weight {
				...
				dist[to] = weight
				// 確定候補キューに格納
				heap.Push(h, Edge{Weight: dist[to], To: to, From: p.From})
			}
		}
	}
	return dist
