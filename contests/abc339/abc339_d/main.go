...
func solve() {
	...
	if c.y1 > c.y2 {
		c.y1, c.x1, c.y2, c.x2 = c.y2, c.x2, c.y1, c.x1
	} else if c.y1 == c.y2 && c.x1 > c.x2 {
		c.y1, c.x1, c.y2, c.x2 = c.y2, c.x2, c.y1, c.x1
	}
	dist := make(map[t]int)
	dist[c] = 0
	...
	for len(que) > 0 {
		...
		que = que[1:]
		for i := 0; i < 4; i++ {
			ny1, nx1, ny2, nx2 := now.y1+dy[i], now.x1+dx[i], now.y2+dy[i], now.x2+dx[i]
			if ny1 >= 0 && ny1 < n && nx1 >= 0 && nx1 < n && s[ny1][nx1] == '#' {
				ny1, nx1 = now.y1, now.x1
			} else {
				if ny1 < 0 {
					ny1 = 0
				}
			...
			if ny2 >= 0 && ny2 < n && nx2 >= 0 && nx2 < n && s[ny2][nx2] == '#' {
				ny2, nx2 = now.y2, now.x2
			} else {
				if ny2 < 0 {
					ny2 = 0
				}
			...
			nt := t{ny1, nx1, ny2, nx2}
			if _, ok := dist[nt]; ok {
				continue
			}
			rnt := t{ny2, nx2, ny1, nx1}
			if _, ok := dist[rnt]; ok {
				continue
			}
			dist[nt] = dist[now] + 1
