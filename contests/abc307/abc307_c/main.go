...
func solve() {
	...
	for i := au; i <= ab; i++ {
		A = append(A, []uint8{})
		for j := al; j <= ar; j++ {
			A[i-au] = append(A[i-au], a[i][j])
		}
	}
	HB, WB := nextInt2()
	b := nexts(HB)
	bl, br, bu, bb := MaxInt, 0, MaxInt, 0
	for i := 0; i < HB; i++ {
		for j := 0; j < WB; j++ {
			if b[i][j] == '#' {
				bl = min(bl, j)
	...
	for ay := 0; ay < HX; ay++ {
		for ax := 0; ax < WX; ax++ {
			for by := 0; by < HX; by++ {
				for bx := 0; bx < WX; bx++ {
					if ax+len(A[0]) > WX || ay+len(A) > HX || bx+len(B[0]) > WX || by+len(B) > HX {
						continue
					}
					flag := true
					temp := make([][]uint8, HX)
					for i := 0; i < HX; i++ {
						temp[i] = make([]uint8, WX)
						for j := 0; j < WX; j++ {
							temp[i][j] = '.'
						}
					}
