...
func findCheapestPrice(n int, flights [][]int, src int, dst int, k int) int {
	...
	for _, v := range flights {
		g[v[0]] = append(g[v[0]], Edge{To: v[1], Weight: v[2]})
	}
	dp := make([][]int, k+2)
	dp[0] = make([]int, n)
	for i := 0; i < n; i++ {
	...
	for i := 0; i <= k; i++ {
		...
		for j := 0; j < n; j++ {
			...
			for _, v := range g[j] {
				if dp[i][v.To] == -1 || dp[i+1][v.To] > dp[i][j]+v.Weight {
					dp[i+1][v.To] = dp[i][j] + v.Weight
				}
			}
		}
	}
