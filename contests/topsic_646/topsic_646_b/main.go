...
func solve() {
	n, s := nextInt2()
	var dfs func(index, remain int, ans []int)
	dfs = func(index, remain int, ans []int) {
		if index == n {
			if remain >= 1 {
				out.PrintIntsLn(append(ans, remain))
			}
			return
		}
		pre := 1
		...
		for i := pre; i*(n-index) <= remain-i; i++ {
			dfs(index+1, remain-i, append(ans, i))
		}
	}
	dfs(1, s, []int{})
}
