...
func solve() {
	...
	copy(a, s)
	for next1 := true; next1; next1 = nextPermutation(a) {
		copy(b, s)
		for next2 := true; next2; next2 = nextPermutation(b) {
			copy(c, s)
			for next3 := true; next3; next3 = nextPermutation(c) {
				ans := make([][]byte, n)
				f := true
				for i := 0; i < n; i++ {
