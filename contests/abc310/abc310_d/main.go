...
func solve() {
	...
	var dfs func(remain []int, teamNum int) int
	dfs = func(remain []int, teamNum int) int {
		if len(remain) == 0 {
			return 0
		}
		teamNum++
		...
		sum := 0
		for bit := 0; bit < (1 << (len(remain) - 1)); bit++ {
			team := []int{remain[0]}
			nextRemain := []int{}
			for i := 0; i < len(remain)-1; i++ {
				if (bit>>i)&1 > 0 {
					team = append(team, remain[i+1])
				} else {
					nextRemain = append(nextRemain, remain[i+1])
				}
			}
			flag := false
			for i := 0; i < len(team); i++ {
				for j := i + 1; j < len(team); j++ {
					if dis[team[i]][team[j]] {
						flag = true
						break
					}
				}
