...
func solve() {
	...
	ave /= float64(d)
	ans := math.MaxFloat64
	var dfs func(groups [][]float64, now int)
	dfs = func(groups [][]float64, now int) {
		if now == n {
		...
		if now != 0 {
			for i := 0; i < len(groups); i++ {
				groups[i] = append(groups[i], float64(w[now]))
				dfs(groups, now+1)
				groups[i] = groups[i][:len(groups[i])-1]
			}
		}
		if d-len(groups) != 0 {
			dfs(append(groups, []float64{float64(w[now])}), now+1)
		}
	}
	dfs([][]float64{}, 0)
	out.Println(ans)
