...
func solve() {
	...
	que := []p{{y: 1, x: 1, s: s[0][0]}}
	snuke := "snuke"
	snukeNum := map[byte]int{
		's': 0,
		'n': 1,
		'u': 2,
		'k': 3,
	...
	for len(que) > 0 {
		...
		que = que[1:]
		for i := 0; i < 4; i++ {
			ny, nx := now.y+dy[i], now.x+dx[i]
			ns := snuke[(snukeNum[now.s]+1)%5]
			if ny < 0 || ny >= h || nx < 0 || nx >= w || vis[ny][nx] || ns != s[ny][nx] {
				continue
			}
			if ny == h-1 && nx == w-1 {
				out.YesNo(true)
			...
			vis[ny][nx] = true
			que = append(que, p{y: ny, x: nx, s: ns})
		}
	}
	out.YesNo(false)
}
