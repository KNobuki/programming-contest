...
func solve() {
	...
	for i := 0; i < n; i++ {
		sxi := sort.Search(A, func(j int) bool {
			return ra[j] < p[i]
		})
		sx := 0
		if sxi < A {
		...
		txi := sort.Search(A, func(j int) bool {
			return a[j] > p[i]
		})
		tx := w
		if txi < A {
		...
		syi := sort.Search(B, func(j int) bool {
			return rb[j] < q[i]
		})
		sy := 0
		if syi < B {
		...
		tyi := sort.Search(B, func(j int) bool {
			return b[j] > q[i]
		})
		ty := h
		if tyi < B {
	...
	ansMin, ansMax := MaxInt, 0
	for _, v := range parts {
		ansMin = min(ansMin, v)
		ansMax = max(ansMax, v)
	}
	if len(parts) < (A+1)*(B+1) {
		out.Printf("0 ")
	} else {
		out.Printf("%d ", ansMin)
	}
