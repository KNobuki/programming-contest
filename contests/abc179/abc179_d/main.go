...
func solve() {
	...
	em := M{0}
	fm := func(a, b M) M {
		return M{madd(a.val, b.val)}
	}
	fa := func(a X, b M) X {
		return X{madd(a.val, b.val)}
	}
	seg := NewSegTreeLazy(n, ex, em, fx, fm, fa)
	seg.Set(0, X{1})
	seg.Build()
	for i := 0; i < n-1; i++ {
		a := seg.Query(i, i+1).val
		for j := 0; j < k; j++ {
			if i+pairs[j].l >= n {
				continue
			}
			seg.Update(i+pairs[j].l, min(i+pairs[j].r+1, n+1), M{a})
		}
	}
	out.Println(seg.Query(n-1, n).val)
}
