...
func solve() {
	...
	for i := 0; i < n; i++ {
		...
		for j := 0; j < n; j++ {
			...
			g[i] = append(g[i], Edge{To: j, Weight: d})
		}
	}
	dist := make([]int, n)
	for i := 1; i < n; i++ {
	...
	for pq.Len() > 0 {
		...
		for _, v := range g[now.To] {
			d := now.Weight + v.Weight*a
			if d < dist[v.To] {
				heap.Push(pq, Edge{To: v.To, Weight: d})
				dist[v.To] = d
			}
		}
	}
	...
	heap.Push(pq, Edge{To: n - 1, Weight: 0})
	for pq.Len() > 0 {
		now := heap.Pop(pq).(Edge)
		if dist2[now.To] != now.Weight {
			continue
		}
		for _, v := range g[now.To] {
			d := now.Weight + v.Weight*b + c
			if d < dist2[v.To] {
				heap.Push(pq, Edge{To: v.To, Weight: d})
				dist2[v.To] = d
			}
